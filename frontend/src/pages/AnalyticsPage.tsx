...
type Overview = { test: Test; enrollmentCount: number; submittedCount: number; completionRate: number; averageScore: number; highestScore: number; averageDurationMinutes: number; distribution: { range: string; count: number }[] };
type LeaderboardRow = { rank: number; student: { name: string; email: string }; score: number; percentile: number; submittedAt?: string; attemptNumber: number };
...
export default function AnalyticsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {!loading && overview && <>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[["Completion", `${overview.completionRate}%`, `${overview.submittedCount} of ${overview.enrollmentCount} enrolled`], ["Average score", `${overview.averageScore}%`, "Across submitted attempts"], ["Highest score", `${overview.highestScore}%`, "Best learner result"], ["Avg. duration", `${overview.averageDurationMinutes} min`, "Time spent per submission"]].map(([label, value, helper]) => <div key={label} className="rounded-lg border bg-white p-5 shadow-sm"><p className="text-sm text-slate-500">{label}</p><p className="mt-2 text-3xl font-bold text-slate-900">{value}</p><p className="mt-2 text-xs text-slate-400">{helper}</p></div>)}</div>
