...
The `/api/health` endpoint should be configured as the service health check. The API process exits during production startup when `JWT_SECRET` is missing, preventing an accidentally insecure deployment.
...
For a hosted deployment, create the MongoDB database first, configure the API environment variables in the hosting provider, deploy the backend, and then deploy the frontend with the API proxy or an equivalent same-origin reverse proxy. Do not commit `.env` files or production credentials.
